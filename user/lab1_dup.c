...
// 心路历程：首先是以为直接用c标准库，后来发现这一整个模拟都在仓库里写好了功能；
// 其次尝试了把父进程中的wait放在执行父进程主函数之前or之后，一个会出现乱码；一个刚开始直接读取了子进程完成的输入的所有数据
// 导致问题。目前是采取子进程全部执行完再执行父进程，但子进程默认生成一位整数，且父进程也只会每次读一个字节
// 这里还没有想到很好的解决办法，是否子进程的每一轮输入之间都要以特殊符号分隔？？这样才能保证通信
// 以及为什么子进程父进程同时执行，答案就是对的？虽然是乱码？
